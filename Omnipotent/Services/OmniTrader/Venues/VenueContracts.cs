...
namespace Omnipotent.Services.OmniTrader.Venues
{
    ...
    /// <summary>
    /// What a channel is currently known to be doing. <see cref="Unknown"/> matters as much as the
    /// others: a channel nobody has called yet has not failed, and reporting it as down produces a
    /// permanently red row that no action can clear.
    /// </summary>
    public enum ChannelState { Unknown = 0, Up = 1, Down = 2, Unsupported = 3 }
    ...
    public sealed class ChannelHealth
    {
        ...
        /// <summary>Set when the venue simply does not offer this channel. A declared limitation is
        /// not a fault, and must never count as an outage.</summary>
        public bool Unsupported { get; init; }

        public ChannelState State =>
            Unsupported ? ChannelState.Unsupported
            : Connected ? ChannelState.Up
            : LastErrorUtc.HasValue ? ChannelState.Down
            : ChannelState.Unknown;

        /// <summary>Never probed — no call has yet exercised this channel in this process.</summary>
        public bool Probed => LastOkUtc.HasValue || LastErrorUtc.HasValue;

        public bool Degraded => State == ChannelState.Down || ConsecutiveFailures > 0;
    }
    ...
    public sealed class VenueHealthSnapshot
    {
        ...
        /// <summary>
        /// A venue is tradeable unless an order-carrying channel has actually *failed*.
        ///
        /// This deliberately treats "never probed" as acceptable. Requiring every such channel to be
        /// positively connected marked Kraken's order path unhealthy purely because nothing had yet
        /// called the directory endpoint in that process — an outage report generated by an absence
        /// of activity rather than by a fault.
        /// </summary>
        public bool OrderPathHealthy =>
            Configured && !Channels.Where(c => c.Channel.Contains("rest", StringComparison.OrdinalIgnoreCase)
                                   ...
                                   .Any(c => c.State == ChannelState.Down);
    }
}
