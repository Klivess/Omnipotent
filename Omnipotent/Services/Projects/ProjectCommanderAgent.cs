...
{browserContract}
- Your computer sleeps when idle and resumes with its installed applications, home files and settings intact. Install needed GUI software with computer_terminal, launch it with computer_launch_app, then use screenshots, window focus, mouse and keyboard to work in it. A browser-helper failure does not mean the desktop or terminal is broken. For infrastructure failures, report the measured failed layer and exact error. The platform coordinates Docker recovery: do not run competing fleet-wide restarts from multiple projects. A stopped docker-desktop-data distro or a failed shell command against it is not proof of corruption; never propose unregistering it without an independently verified diagnosis and a data-preserving recovery plan.
...
- NEED AN EXISTING SIGN-IN? Use browser op=take_session with sourceAgentId (use 'commander' for the Commander, or an ID from YOUR TEAM) to copy that Project agent's persistent browser session into your desktop and restart Chromium. It is direct and project-scoped: session values never appear in your tool result. The target profile is replaced, so take the session only when you actually need that account; inspect the site afterwards because a provider can still reject a moved session.
...
Token budget: ${project.TokenBudgetUsd:0.##}. Real-money budget: ${project.MoneyBudgetUsd:0.##}. Autonomous per-action money threshold: ${project.MoneyAutonomousThresholdUsd:0.##}.{planning}";
        ...
        public static List<HFWrapper.HFTool> BuildCoreToolDefinitions()
        {
            ...
            return new List<HFWrapper.HFTool>
            {
                ...
                Tool("request_human", "Ask a human (Klives) to clear a genuinely human-only obstacle such as SMS/phone verification, an identity check, or a physical action. Try browser op=solve_challenge FIRST for any captcha; if free recovery is exhausted on an essential page, provide its diagnosis and use this same desktop for takeover. Provide either 'what' or a structured title/description; provider rate limits, infrastructure debugging, tool execution, file reads, and ordinary email retrieval are not human-only.",
                    Obj(new
                    {
        ...
        public static List<HFWrapper.HFTool> BuildComputerToolDefinitions(bool visionEnabled = true)
        {
            ...
            tools.Add(Tool("computer_browser_action",
                "Operate the SAME persistent visible Chromium session entirely through structured text/CLI control. Use inspect mode=controls, then target by its opaque ref or by semantic fields. Handles forms, open shadow roots, same/cross-origin frames, history, tabs, waits and scrolling without model vision. Every result returns bounded URL/title/tab/dialog state for verification. fill/type read the field back and FAIL if the value did not land, so a form you were told is filled really is. op=take_session copies a named Project agent's persistent browser sign-in to THIS desktop and restarts Chromium; raw session values never enter the result, and it replaces this desktop's prior profile. op=solve_challenge waits for the free CAPTCHA extension and reports observed response state. It needs no account/key, has a shared IP quota, and cannot guarantee success. Follow its recovery diagnosis; do not blindly repeat the solve or buy accounts. A click blocked by a cookie wall or modal clears it automatically and retries; op=dismiss_overlays sweeps them page-wide. op=script is a last-resort live-page escape hatch and rejects obvious secret/storage/network-exfiltration access; never embed credentials in script.",
                ProjectObj(new
                {
                    op = new
                    {
                        type = "string",
                        @enum = new[] { "click", "fill", "type", "select", "check", "uncheck", "focus", "hover", "scroll_into_view", "scroll", "press", "wait", "back", "forward", "reload", "activate_tab", "close_tab", "script", "solve_challenge", "dismiss_overlays", "take_session" },
                    ...
                    timeoutMs = ProjectNum("Bounded wait/script timeout, 100-120000; default 15000. For free solve_challenge: 1000-120000, default 90000."),
                    sourceAgentId = ProjectStr("For take_session: source roster agent ID (or 'commander'). Its browser sign-in replaces this desktop's browser profile; no session value is returned."),
