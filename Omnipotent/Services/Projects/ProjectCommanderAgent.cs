...
Token budget: ${project.TokenBudgetUsd:0.##}. Real-money budget: ${project.MoneyBudgetUsd:0.##}. Autonomous per-action money threshold: ${project.MoneyAutonomousThresholdUsd:0.##}.{planning}";
        ...
        public static List<HFWrapper.HFTool> BuildComputerToolDefinitions(bool visionEnabled = true)
        {
            ...
            tools.Add(Tool("computer_browser_action",
                "Operate the SAME persistent visible Chromium session entirely through structured text/CLI control. Use inspect mode=controls, then target by its opaque ref or by semantic fields. Handles forms, open shadow roots, same/cross-origin frames, history, tabs, waits and scrolling without model vision. Every result returns bounded URL/title/tab/dialog state for verification. fill/type read the field back and FAIL if the value did not land, so a form you were told is filled really is. op=solve_challenge waits for the free CAPTCHA extension and reports observed response state. It needs no account/key, has a shared IP quota, and cannot guarantee success. Follow its recovery diagnosis; do not blindly repeat the solve or buy accounts. A click blocked by a cookie wall or modal clears it automatically and retries; op=dismiss_overlays sweeps them page-wide. op=script is a last-resort live-page escape hatch and rejects obvious secret/storage/network-exfiltration access; never embed credentials in script.",
                ProjectObj(new
                {
                    op = new
                    {
