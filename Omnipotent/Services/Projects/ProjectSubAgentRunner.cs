                    ...
                    if (resp.PromptTokens > 0 || resp.CompletionTokens > 0)
                    {
                        ...
                        await parent.Budget.RecordTokenSpendAsync(
                            ...
                            new ProjectTokenUsageContext
                            {
                                ...
                                QueueDurationMs = resp.QueueDurationMs,
                                ProviderDurationMs = resp.ProviderDurationMs,
                                LatencyBreakdownAvailable = resp.LatencyBreakdownAvailable,
...
- For browser/GUI work: inspect structured state, take one action, wait for its expected text/DOM/OCR postcondition, then inspect again. Do not retry blind actions. For a CAPTCHA, call browser op=solve_challenge once: it uses the free browser extension without an account/key. Its quota is shared by public IP. Do not register paid solvers or retry an unchanged challenge; use an official API/alternative route, or request_human on the same desktop when an essential page remains unresolved. Verify the form result after a response appears. SMS/phone, identity, hardware-key and physical verification may also need request_human.
