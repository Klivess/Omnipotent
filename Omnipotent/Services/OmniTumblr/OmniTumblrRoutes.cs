...
namespace Omnipotent.Services.OmniTumblr
{
    ...
    {
        private readonly OmniTumblr p;

        public OmniTumblrRoutes(OmniTumblr parent)
        {
            p = parent;
        }
        ...
        {
            await p.CreateAPIRoute("/omnitumblr/accounts/add", async (req) =>
            {
                try
                {
                    var body = JsonConvert.DeserializeObject<OmniTumblrAddAccountRequest>(req.userMessageContent);
                    if (body == null)
                    {
            ...
            await p.CreateAPIRoute("/omnitumblr/accounts/updateSettings", async (req) =>
            {
                try
                {
                    var body = JsonConvert.DeserializeObject<OmniTumblrUpdateAccountSettingsRequest>(req.userMessageContent);
                    if (body == null)
                    {
                        await req.ReturnResponse("Invalid request body", code: HttpStatusCode.BadRequest);
                        return;
            ...
            await p.CreateAPIRoute("/omnitumblr/accounts/delete", async (req) =>
            {
                try
                {
                    var body = JsonConvert.DeserializeObject<OmniTumblrDeleteAccountRequest>(req.userMessageContent);
                    if (body == null)
                    {
                        await req.ReturnResponse("Invalid request body", code: HttpStatusCode.BadRequest);
                        return;
            ...
            await p.CreateAPIRoute("/omnitumblr/accounts/list", async (req) =>
            {
            ...
            await p.CreateAPIRoute("/omnitumblr/accounts/live", async (req) =>
            {
                try
                {
            ...
            await p.CreateAPIRoute("/omnitumblr/accounts/liveAnalytics", async (req) =>
            {
                try
                {
            ...
            await p.CreateAPIRoute("/omnitumblr/posts/schedule", async (req) =>
            {
                try
                {
                    OmniTumblrScheduleRequest? body = null;
                    ...
                    if (hasUploadBytes)
                    {
                        body = new OmniTumblrScheduleRequest
                        {
                            AccountId = req.userParameters?["accountId"],
                            DispatchMode = ParseEnumOrDefault(req.userParameters?["dispatchMode"], OmniTumblrDispatchMode.SingleAccount),
                    ...
                    {
                        body = JsonConvert.DeserializeObject<OmniTumblrScheduleRequest>(req.userMessageContent);
                    }
            ...
            await p.CreateAPIRoute("/omnitumblr/posts/deleteFromTumblr", async (req) =>
            {
                try
                {
                    var body = JsonConvert.DeserializeObject<OmniTumblrDeletePostRequest>(req.userMessageContent);
                    if (body == null)
                    {
                        await req.ReturnResponse("Invalid request body", code: HttpStatusCode.BadRequest);
                        return;
                    ...
                    bool ok = await p.DeleteTumblrPost(body);
            ...
            await p.CreateAPIRoute("/omnitumblr/posts/list", async (req) =>
            {
                int take = 500;
            ...
            await p.CreateAPIRoute("/omnitumblr/analytics/overview", async (req) =>
            {
            ...
            await p.CreateAPIRoute("/omnitumblr/logs/events", async (req) =>
            {
                int take = 500;
                if (int.TryParse(req.userParameters?["take"], out int parsedTake) && parsedTake > 0)
                {
            ...
            await p.CreateAPIRoute("/omnitumblr/health", async (req) =>
            {
                await req.ReturnResponse(JsonConvert.SerializeObject(new
                {
                    Service = "OmniTumblr",
